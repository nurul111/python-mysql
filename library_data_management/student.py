...
class Search(Tk):
    def __init__(self):
        ...
        l1=Label(self,text="Search Student",bg='#005D5C',fg='yellow', font=("Courier new",20,'bold')).place(x=305,y=20)
        l = Label(self, text="Search By",bg='#005D5C',fg='white', font=("Courier new", 15, 'bold')).place(x=180, y=60)
        ...
        def ge():
            if (len(self.entry.get())) == 0:
                ...
                try:
                    ...
                    self.mycursor = self.conn.cursor()
                    id = self.entry.get()
                    self.mycursor.execute("Select * from student where student_id = %s", [id])
                    pc = self.mycursor.fetchall()
                    if pc:
                        insert(pc)
                    else:
        ...
        self.b= Button(self,text="Find",width=8,bg='#005D5C',fg='white',font=("Courier new",12,'bold'),command= ge )
        self.b.place(x=380,y=150)
        self.combo=ttk.Combobox(self,textvariable=g,values=["Name","ID"],width=40,state="readonly")
        self.combo.place(x = 310, y = 65)
        self.entry = Entry(self,textvariable=f,width=43)
        self.entry.place(x=310,y=105)
        self.la = Label(self, text="Enter",bg = '#005D5C',fg='white', font=("Courier new", 15, 'bold')).place(x=180, y=100)
        ...
        self.listTree.heading("#0", text='Student ID', anchor='center')
        self.listTree.column("#0", width=150, anchor='w')
        ...
        self.listTree.column("Phone Number", width=150, anchor='center')
